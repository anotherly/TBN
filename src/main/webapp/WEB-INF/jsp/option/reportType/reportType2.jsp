...
<%-- <c:forEach var="list" items="${allTypeCode2}" step="1" varStatus="status1">
	...
	<c:forEach var="list" items="${userTypeCode2}" step="1" varStatus="status2">
		<li id="liTypeCode_${list.basScod }">
			<input type="checkbox" id="chkTypeCode2" name="chkTypeCode2" value="${list.basScod}" />
			<span onclick="getTypeCode('${list.basScod}','3', $(this),'${list.basLcod}')" ondblclick="modTypeCode('${list.basScod}','${list.basName }','2',$(this))" style="cursor: pointer;">
       			${list.basName}
       		</span>
		</li>
	</c:forEach>
