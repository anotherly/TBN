...
<%-- <c:forEach var="list" items="${allTypeCode2}" step="1" varStatus="status1">
	...
	<c:forEach var="list" items="${userTypeCode2}" step="1" varStatus="status2">
		<li id="liTypeCode_${list.areaSubCode}">
			<input type="checkbox" id="chkTypeCode2" name="chkTypeCode2" value="${list.areaSubCode}" />
			<span onclick="getTypeCode('${list.areaSubCode}','3', $(this),'${list.areaCode}')" ondblclick="modTypeCode('${list.areaSubCode}','${list.areaName}','2',$(this))" style="cursor: pointer;">
       			${list.areaName}
       		</span>
		</li>
	</c:forEach>
