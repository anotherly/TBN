...
public class FileUploadSave implements ApplicationContextAware{
	...
	/*채택*/
	// 여러 개의 파일 업로드
    public List<eFileVO> fileUploadMultiple(List<MultipartFile> files, String inputPath,EventVO inputVo) {
    	List<eFileVO> fileList = new ArrayList<>();
    	try {
	        // 1. 파일 저장 경로 설정 : 실제 서비스되는 위치(프로젝트 외부에 저장)
	        // 여러 개의 원본 파일을 저장할 리스트 생성
    		
    		
    		String SlicePath = inputPath.split("eventFile/")[1];
...
//		            	}
	            ...
	            fvo.setFileDir(inputPath.split("eventFile")[1]);
	            fvo.setFileName(originalFileName);
	            fvo.setEventId(SlicePath);
				fileList.add(fvo);
	            i++;
	        }
    	}catch (Exception e) {
	...
	public void deleteFile (List<EventVO> fileList, String inputPath){      
	...
	// 파일 일괄삭제
	public void deleteFileOne(String inputPath , String fileName) {
